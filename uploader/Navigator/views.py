...
def index(req_path):
    ...
    # Get inherited permission, if any
    inherited_permission = perms.get_inherited_permission_of_directory(abs_path)

    return render_template('files.html', entries=entries, req_path=req_path, back_path=back_path, permissions_filename=permissions.PERMISSION_METADATA_FILENAME, inherited_permission=inherited_permission)
